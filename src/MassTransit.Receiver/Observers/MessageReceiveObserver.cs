...
public class MessageReceiveObserver : IReceiveObserver
{
    ...
    {
        Console.WriteLine(string.Concat(_logger, ": A fault is consumed for type ", context.Message.GetType(),
                                        ". Consumer type: ", consumerType, ", exception message: ", exception.Message));
    ...
    public async Task PostConsume<T>(ConsumeContext<T> context, TimeSpan duration, string consumerType) where T : class
    {
        Console.WriteLine(string.Concat(_logger, ": A message is consumed for type ", context.Message.GetType(),
                                        ". Consumer type: ", consumerType, ", duration in millis: ",
    ...
    public async Task PostReceive(ReceiveContext context)
    {
        Console.WriteLine(string.Concat(
                              _logger,
                              ": A message is past the receive stage. Has been delivered to at least one consumer: ",
                              context.IsDelivered));
    ...
    public async Task PreReceive(ReceiveContext context)
    {
        Console.WriteLine(string.Concat(
                              _logger,
                              ": A message is before the receive stage. Has been delivered to at least one consumer: ",
                              context.IsDelivered));
        await context.ReceiveCompleted;
    }
